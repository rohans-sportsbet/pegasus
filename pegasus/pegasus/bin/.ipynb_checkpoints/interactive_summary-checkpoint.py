...
def _get_model_dir(model):
    if model is not None:
        model_dir = f"ckpt/pegasus_ckpt/{model}"
    else:
        model_dir = "ckpt/pegasus_ckpt"
    return model_dir

        
def _format_raw_input(raw):
    with tf.io.TFRecordWriter("pegasus/data/testdata/interactive_test.tfrecord") as writer:
        example = tf.train.Example(
            features=tf.train.Features(
                feature={
                    "inputs": tf.train.Feature(bytes_list=tf.train.BytesList(value=[raw.encode('utf-8')])),
                    "targets": tf.train.Feature(bytes_list=tf.train.BytesList(value=["no targets".encode('utf-8')])),
                }
            )
        )
...
def main(_):
  model_dir = _get_model_dir(FLAGS.model)
  _format_raw_input(FLAGS.input)
  if not FLAGS.wait and not tf.train.checkpoint_exists(model_dir):
    raise ValueError(("Checkpoints %s doesn't exist " % model_dir,
  ...
  while True:
    if tf.train.checkpoint_exists(model_dir):
      ...
      if tf.io.gfile.isdir(model_dir):
        eval_dir = model_dir
        if FLAGS.best:
          checkpoint_id = _get_best_checkpoint_id(model_dir)
      ...
      params = registry.get_params("interactive_transformer")(FLAGS.param_overrides, FLAGS.model)
      ...
      params.interactive_eval(predictions, eval_dir, global_step, eval_tag,
...
if __name__ == "__main__":
  flags.mark_flags_as_required(["input"])
